...
| Bundle adjustment | Sparse Levenberg–Marquardt with the Schur complement and Huber loss | `src/vision/ba.js` |
...
| Fusion | Truncated signed distance volume with per-voxel colour | `src/mesh/tsdf.js` |
...
Browsers do not report focal lengths. Each camera is mapped to a lens type (wide, ultra-wide, telephoto, front) from its label, which sets its field of view. If a phone reports generic names ("camera2 0, facing back") check the mapping under **Settings**; a wrong field of view degrades accuracy. Imported photos use the 35 mm-equivalent focal length from EXIF when present.
